...
|int lstat(const char *pathname, const stat *statbuf))|심벌릭 링크 정보 검색|pathname : 기존 파일의 경로<br>statbuf : 새로 생성할 링크의 경로|성공하면 0, 실패하면 -1|
|ssize_t readlink(const char *pathname, char *buf, size_t bufsiz))|심벌릭 링크 내용 읽기|pathname : 기존 파일의 경로<br>buf : 새로 생성할 링크의 경로<br>buf size : |성공하면 0, 실패하면 -1|
|char *realpath(const char *path, char *resolved_path))|심벌릭 링크 원본 파일 경로 읽기|path : 기존 파일의 경로<br>resolved_path : 새로 생성할 링크의 경로|성공하면 0, 실패하면 -1|
|int unlink(const char *pathname))|링크 끊기|pathname : 기존 파일의 경로|성공하면 0, 실패하면 -1|
