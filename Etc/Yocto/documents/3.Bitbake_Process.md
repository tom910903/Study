# 3장. **Bitbake 순서**
  ...
  * [3-4 설정, 컴파일, 스테이징](#3-3-설정-컴파일-스테이징)
...
## 3-3 Patching  
  ...
  `do_patch()` 태스크는 레시피의 `SRC_URI` 문과 `FILESPATH` 변수를 사용하여 적용 가능한 패치 파일을 찾는다.  
...
## 3-4 설정, 컴파일, 스테이징  
  소스 코드가 패치된 후 BitBake는 소스 코드를 구성하고 컴파일하는 작업을 실행한다.  
  컴파일이 완료되면 패키징을 준비하기 위해 파일을 보류 영역(스테이징)에 복사합니다.
  ...
  * `do_configure`: 빌드 중인 소프트웨어의 빌드 시간 및 configuration 옵션을 활성화 및 비활성화하여 소스를 구성한다.  
  ...
  `do_configure` 태스크가 처리하는 configuration은 레시피에 의해 빌드되는 소스 코드에 대한 구성에만 해당됩니다.
  `autotools` 클래스를 사용하는 경우 `EXTRA_OECONF` 또는 `PACKAGECONFIG_CONFARGS` 변수를 사용하여 추가 구성 옵션을 추가할 수 있다.  
  * `do_compile`: configuration이 완료되면 `BitBake`는 `do_compile` 작업을 사용하여 소스를 컴파일한다.  
  컴파일은 `B` 변수가 가리키는 디렉토리에서 이루어진다.  
  `B` 디렉터리는 기본적으로 `S` 디렉터리와 동일하다.  
  * `do_install`: 컴파일이 완료되면 BitBake는 `do_install` 작업을 실행한다.  
