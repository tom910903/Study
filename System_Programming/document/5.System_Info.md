...
|long fpathcof(int fd, int name)|파일과 디렉터리 자원 검색|fd : 파일 기술자</br>name : 검색할 정보를 지정하는 상수|성공하면 정수, 실패하면 -1|
|long pathcof(const char *path, int name)|파일과 디렉터리 자원 검색|path : 파일이나 디렉터리  경로</br>name : 검색할 정보를 지정하는 상수)|성공하면 정수, 실패하면 -1|
