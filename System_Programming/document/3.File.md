...
|:--:|--|--|--|
|int link(const char *oldpath, const char *newpath))|하드 링크 생성|oldpath : 기존 파일의 경로<br>newpath : 새로 생성할 하드 링크의 경로|성공하면 0, 실패하면 -1|
|int symlink(const char *target, const char *linkpath))|심벌릭 링크 생성|target : 기존 파일의 경로<br>linkpath : 새로 생성할 심벌릭 링크의 경로|성공하면 0, 실패하면 -1|
|int lstat(const char *pathname, const stat *statbuf))|심벌릭 링크 정보 검색|pathname : 심벌릭 링크의 경로<br>statbuf : 파일 정보를 저장할 포인터|성공하면 0, 실패하면 -1|
|ssize_t readlink(const char *pathname, char *buf, size_t bufsiz))|심벌릭 링크 내용 읽기|pathname : 심벌릭 링크의 경로<br>buf : 읽어올 내용을 저장할 버퍼<br>buf size : 버퍼의 크기|성공하면 읽어온 데이터의 크기(바이트), 실패하면 -1|
|char *realpath(const char *path, char *resolved_path))|심벌릭 링크 원본 파일 경로 읽기|path : 심벌릭 링크의 경로명<br>resolved_path : 경로를 저장할 ㅂ버퍼 주소|성공하면 실제 경로명이 저장된 주소, 실패하면 NULL|
|int unlink(const char *pathname))|링크 끊기|pathname : 삭제할 링크의 경로|성공하면 0, 실패하면 -1|
